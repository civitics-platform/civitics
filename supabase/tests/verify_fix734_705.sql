...
--   3. reconcile_recipient_count() is now FR-BASED (FIX-736 superseded the FIX-734
--      revert): it recomputes counts from financial_relationships (the complete
--      source) and DOES zero a genuine orphan that has no donation FR. (The old
--      EC-based zeroing was unsound because EC donation edges are incomplete for
--      individual→committee; FR is complete, so FR-based zeroing is sound. Routine
--      maintenance now lives in the totals incremental + the FR sweep folded into
--      reconcile_financial_entity_totals — see verify_fix736.sql.)
--
...
\echo '── test 3: recipient_count reconcile is FR-BASED (FIX-736: sound zeroing) ──'
-- ORPHAN_DONOR (...01) has recipient_count=5 and NO donation FR anywhere — a
-- GENUINE orphan. The FR-based reconcile (FIX-736 repurposed reconcile_recipient_
-- count from EC-bump-only to an FR two-pass) zeroes it — sound because FR is
-- complete (unlike EC). LIVE_DONOR (...03) has 1 donation FR to ...04, so the
-- bump recomputes COUNT(DISTINCT to_id)=1 (unchanged).
...
BEGIN
  ...
  IF orphan_cnt IS DISTINCT FROM 0 THEN
    RAISE EXCEPTION 'FAIL FR reconcile did not zero a no-FR orphan (got %, want 0 — FR-based zeroing is sound, FIX-736)', orphan_cnt;
  END IF;
  IF live_cnt IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FAIL live recipient_count moved: got %, want 1', live_cnt; END IF;
  RAISE NOTICE 'PASS test3: FR-based — no-FR orphan zeroed (0), live count kept (1)';
END $$;
