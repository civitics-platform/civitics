...
import { currentCouncilMemberPersonIds } from "./mappers";
import type { LegistarEventItem, MetroConfig } from "./types";
import {
  upsertBodiesBatch,
  upsertPersonsBatch,
  reconcileCouncilBody,
  upsertMattersBatch,
  upsertEventsBatch,
...
// `councilBodyId` is the Legistar BodyId of the metro's real primary legislative
// body (FIX-471). Verified against the live Legistar API: Seattle "City Council"
// =138, SF "Board of Supervisors"=1, Austin "City Council"=138. Hardcoded because
// BodyTypeName is too unreliable to pick it (it mis-picked Austin's agenda variant
// and SF has no `municipal_council`-typed body at all). Adding a new metro? Look
// up its council BodyId at webapi.legistar.com/v1/<client>/Bodies.
const METRO_CLIENTS: Array<{ client: string; name: string; slug: string; councilBodyId: number }> = [
  { client: "seattle",     name: "Seattle",       slug: "seattle",       councilBodyId: 138 },
  { client: "sfgov",       name: "San Francisco", slug: "san-francisco", councilBodyId: 1   },
  { client: "austintexas", name: "Austin",        slug: "austin",        councilBodyId: 138 },
];
...
async function runMetro(
  ...
  // Recency guard applies to the full ingest only. --membership-only is the
  ...
  // parked everyone on an agenda variant.
  const councilBodyId: string | undefined = bodyRes.bodyIdMap.get(config.councilBodyId);

  // Current membership from OfficeRecords — the real signal (PersonActiveFlag
  // flags most of the all-time roster and floods the council page).
  const officeRecords = await api.fetchOfficeRecords();
  const councilSeats = bodies.find((b) => b.BodyId === config.councilBodyId)?.BodyNumberOfMembers ?? null;
  ...
  let personRes: Awaited<ReturnType<typeof upsertPersonsBatch>> = {
    personIdMap: new Map(),
    inserted: 0, failed: 0, reactivated: 0, deactivated: 0, rekeyed: 0,
  };
  if (!councilBodyId) {
  ...
  // no cursor bump — those belong to the full ingest).
  if (membershipOnly) {
    return {
      matters: 0, meetings: 0, agendaItems: 0, votes: 0,
      officials: personRes.personIdMap.size, bodies: bodyRes.bodyIdMap.size,
    };
  }
...
export async function runLegistarPipeline(): Promise<PipelineResult> {
  ...
  // self-healing backfill path — re-keys existing officials onto the real
  // council body and refreshes is_active from current membership, skipping the
  // slow per-event matter/vote fetches.
  ...
  try {
    ...
    for (const config of metros) {
      try {
        const r = await runMetro(db, config, force, membershipOnly);
