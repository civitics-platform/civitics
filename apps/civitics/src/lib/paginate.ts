...
/**
 * `fetchChunkedByIds` and its two ceilings now live in `@civitics/db`
 * (`read-helpers.ts`) so `packages/` call sites can reach them too — FIX-1037.
 * Re-exported here because ~34 `apps/` sites and `paginate.test.ts` import them
 * from `@/lib/paginate`, and moving a helper is not a reason to touch 34 import
 * lines.
 */
export {
  fetchChunkedByIds,
...
export type { ChunkedFetchResult } from "@civitics/db";
