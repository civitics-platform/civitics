...
// Body: { reason, note? } → content_flags(content_type='entity_comment').
// Idempotent (unique content_type+content_id+user_id). The flag-autotrip trigger
// collapses the comment to needs_review at >= 3 unresolved flags.
export async function POST(
  ...
  try {
    ...
    if (comment.author_id === user.id) {
      return NextResponse.json({ error: "You can't flag your own comment" }, { status: 400 });
    }
    ...
    // Idempotent insert (unique constraint on content_type+content_id+user_id).
